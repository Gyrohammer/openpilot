...
class CarInterface(CarInterfaceBase):
    ...
    def _update(self, c):
        ...
        ) > CarControllerParams.DEFAULT_MIN_STEER_SPEED and ret.vEgo < (
            self.CP.minSteerSpeed + 1.0
        ):
            self.low_speed_alert = True
        elif ret.vEgo > (self.CP.minSteerSpeed + 2.0):
        ...
        ret.customStockLong = self.CS.update_custom_stock_long(
            self.CC.cruise_button,
            self.CC.final_speed_kph,
            self.CC.target_speed,
            self.CC.v_set_dis,
            self.CC.speed_diff,
            self.CC.button_type,
        )
